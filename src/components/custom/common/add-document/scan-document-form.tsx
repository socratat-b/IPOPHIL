...
import { docClassificationEnum, DocumentDetails } from '@/lib/dms/schema'
...
export const ScanDocumentForm: React.FC<ScanDocumentFormProps> = ({ onSubmit, onClose, actionType }) => {
    ...
    const handleScan = async () => {
        ...
        try {
            ...
            if (foundDoc) {
                // Map classification to the expected enum value
                const classificationEnum = docClassificationEnum.options.find(
                    (option) => option === foundDoc.classification
                ) || 'simple'

                const mappedDetails: DocumentDetails = {
                    detail_id: foundDoc.id,
                    document_code: foundDoc.code,
                    document_name: foundDoc.title,
                    classification: classificationEnum as typeof docClassificationEnum['_type'],
                    type_id: foundDoc.type,
                    created_by: foundDoc.created_by,
                    created_at: foundDoc.date_created,
                    updated_at: foundDoc.date_created,
                }
                setDocumentDetails(mappedDetails)
            ...
            toast.error(`Error scanning document: ${error}`)
            setDocumentDetails(null)
        } finally {
            setIsLoading(false)
        }
            ...
            <Card>
                ...
                <CardContent className='space-y-4'>
                    ...
                    <ol className='text-sm list-decimal list-inside space-y-2'>
                        ...
                        <li>Wait for the scanner to detect and fill the code in the input box. If successful, details will be shown.</li>
                        <li>If the scanner fails, manually input the code into the input box.</li>
                    </ol>
                        ...
                        <div className='space-y-4'>
                            <div className='grid grid-cols-2 gap-4'>
                                <div>
                                    ...
                                    <Input value={documentDetails.document_code} readOnly className='mt-1' />
                                </div>
                                <div>
                                    <label className='text-sm font-medium'>Name</label>
                                    <Input value={documentDetails.document_name} readOnly className='mt-1' />
                                </div>
                                <div>
